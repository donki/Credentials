...
internal static class Program
{
    private static int Main(string[] args)
    {
        try
        {
            ...
            var root = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "sOCCredentials", "app");
            var target = Path.Combine(root, version);
            var exe = Path.Combine(target, "Credentials.exe");
        ...
        {
            MessageBox(IntPtr.Zero, "No se ha podido arrancar sOC Credentials:\n" + ex.Message, "sOC Credentials", 0x10);
            return 1;
        }
    }
