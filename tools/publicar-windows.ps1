...
.DESCRIPTION
    1. Publica la aplicacion MAUI/WinUI sin empaquetar (carpeta con Credentials.exe y sus ficheros).
    2. La comprime en Launcher\app.zip y compila el lanzador nativo (NativeAOT), que lleva ese zip
       dentro y lo desempaqueta en %LOCALAPPDATA%\sOCCredentials\app\<version> al arrancar. WinUI no
    ...
    Todo sale en bin\windows\: sOCCredentials.exe, sOCCredentials-<version>.zip (la carpeta, por si
    alguien la quiere suelta) y sOCCredentials-<version msix>.msix.
.EXAMPLE
...
$proyecto = Join-Path $raiz "Credentials.csproj"
...
Copy-Item $zip (Join-Path $salida "sOCCredentials-$version.zip") -Force
...
$exe = Get-ChildItem (Join-Path $launcher "bin\Release\net10.0\win-x64\publish") -Filter sOCCredentials.exe | Select-Object -First 1
Copy-Item $exe.FullName (Join-Path $salida "sOCCredentials.exe") -Force
Remove-Item $zip -Force
"Lanzador: $(Join-Path $salida 'sOCCredentials.exe') ($([math]::Round($exe.Length/1MB,1)) MB)"
...
if ($Msix) {
    ...
    $nombre = $paquete.Name -replace '^Credentials_([\d\.]+)_x64\.msix$', 'sOCCredentials-$1.msix'
